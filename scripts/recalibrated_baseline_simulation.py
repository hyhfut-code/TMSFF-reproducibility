...
def compute_fuel_l_per_100km(speed: np.ndarray, acceleration: np.ndarray, distance_m: float) -> float:
    """Eqs. (36)--(37): mL/s, left rectangles, and the evaluated distance."""
    speed = np.maximum(np.asarray(speed, dtype=float), 0.0)
    acceleration = np.asarray(acceleration, dtype=float)
    rate_ml_s = 0.1569 + 0.0245 * speed + 0.00074 * speed**2
    rate_ml_s += np.where(acceleration > 0.0, 0.065 * acceleration * speed, 0.0)
    if distance_m <= 0.0 or len(speed) < 2:
        raise ValueError("A positive evaluated distance and at least two states are required")
    fuel_l = float(np.sum(rate_ml_s[:-1]) * DT * 1e-3)
    return 100.0 * fuel_l / (distance_m * 1e-3)
...
def compute_metrics(follower: pd.DataFrame) -> dict[str, float]:
    ...
    distance_m = follower["s_m"].iloc[-1] - follower.loc[mask, "s_m"].iloc[0]
    return {
...
def plot_dynamic_response(
    ...
    figure, axes = plt.subplots(2, 2, figsize=(7.2, 5.05), dpi=600, sharex=True)
    axes[0, 0].plot(
        leader["time_s"],
        leader["leader_v_mps"],
        color="#222222",
...
def plot_normalized_metrics(metrics: pd.DataFrame, output_path: Path) -> None:
    ...
    figure, axis = plt.subplots(figsize=(6.03, 3.2), dpi=600)
...
def plot_disturbance_heatmaps(timeseries: pd.DataFrame, output_path: Path) -> None:
    ...
    figure, axes = plt.subplots(3, 1, figsize=(5.82, 4.0), dpi=600, sharex=True)
...
def plot_amplification_ratios(ratios: pd.DataFrame, output_path: Path) -> None:
    ...
    figure, axis = plt.subplots(figsize=(4.755, 3.0), dpi=600)
    for model in MODELS:
...
def main() -> None:
    ...
    parser.add_argument("fair_calibration_runs_csv", type=Path)
    parser.add_argument("output_dir", type=Path)
    parser.add_argument("--seed", type=int, default=42)
    args = parser.parse_args()
    args.output_dir.mkdir(parents=True, exist_ok=True)

    parameters = load_seed_parameters(args.fair_calibration_runs_csv, args.seed)
    ...
    metrics.to_csv(
        args.output_dir / "recalibrated_baseline_metrics.csv",
        index=False,
        encoding="utf-8-sig",
    )
    timeseries.to_csv(
        args.output_dir / "recalibrated_baseline_timeseries.csv",
        index=False,
        encoding="utf-8-sig",
    )
    leader.to_csv(
        args.output_dir / "recalibrated_baseline_leader.csv",
        index=False,
        encoding="utf-8-sig",
    )
    plot_dynamic_response(
        leader,
        followers,
        args.output_dir / "Fig_21_recalibrated.png",
    )
    plot_normalized_metrics(
        metrics,
        args.output_dir / "Fig_22_recalibrated.png",
    )
    ...
    disturbance_series.to_csv(
        args.output_dir / "recalibrated_disturbance_timeseries.csv",
        index=False,
        encoding="utf-8-sig",
    )
    amplification_ratios.to_csv(
        args.output_dir / "recalibrated_amplification_ratios.csv",
        index=False,
        encoding="utf-8-sig",
    )
    plot_disturbance_heatmaps(
        disturbance_series,
        args.output_dir / "Fig_15_recalibrated.png",
    )
    plot_amplification_ratios(
        amplification_ratios,
        args.output_dir / "Fig_16_recalibrated.png",
    )
